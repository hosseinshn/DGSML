...
def train_MAML(args, FT, CLS, meta_train_idx, meta_test_idx, ce_loss, optimizer2, batImageGenTrains, batImageGenTrains_metatest):
    beta0 = args.SSL_coef
    ...
    for i in meta_train_idx:
        ...
        x_subset_a = Variable(x_subset_a, requires_grad=False)
        y_subset_a = Variable(y_subset_a, requires_grad=False).long()

        x_subset_ua = torch.from_numpy(domain_a_ux.astype(np.float32))
        y_subset_ua = torch.from_numpy(domain_a_uy.astype(np.int64))
        x_subset_ua = Variable(x_subset_ua, requires_grad=False)
        y_subset_ua = Variable(y_subset_ua, requires_grad=False).long()

        feat_a = FT.features(x_subset_a)
        logits_a = CLS(feat_a)
        feat_ua = FT.features(x_subset_ua)
        logits_ua = CLS(feat_ua)
        l_task = ce_loss(logits_a, y_subset_a)

        unlab_class = torch.from_numpy(np.argmax(torch.nn.functional.softmax(logits_ua, dim=1).detach().numpy(), axis=-1).astype(np.int64))    
        ctr_l = class_centroid(args, feat_a, y_subset_a)       
        score = scoring(torch.nn.functional.softmax(logits_ua, dim = 1))        
        m_feat_ua = score.view(-1,1)*feat_ua
    ...
    for j in meta_test_idx:
        ...
        x_subset_b = Variable(x_subset_b, requires_grad=False)
        y_subset_b = Variable(y_subset_b, requires_grad=False).long()

        x_subset_ub = torch.from_numpy(domain_b_ux.astype(np.float32))
        y_subset_ub = torch.from_numpy(domain_b_uy.astype(np.int64))
        x_subset_ub = Variable(x_subset_ub, requires_grad=False)
        y_subset_ub = Variable(y_subset_ub, requires_grad=False).long()       
     
        feat_b = FT.features(x_subset_b)
        feat_ub = FT.features(x_subset_ub)

        logits_b = CLS(feat_b, fast_weights_CLS)
        logits_ub = CLS(feat_ub, fast_weights_CLS)

        unlab_class_b = torch.from_numpy(np.argmax(torch.nn.functional.softmax(logits_ub, dim=1).detach().numpy(), axis=-1).astype(np.int64))
        ctr_l_b = class_centroid_lu(args, feat_b, y_subset_b)

        score_b = scoring(torch.nn.functional.softmax(logits_ub, dim=1))
        m_feat_ub = score_b.view(-1,1)*feat_ub
        cat_F_lu_b, cat_Y_lu_b = cat(feat_b, y_subset_b, m_feat_ub, unlab_class_b)
        ctr_lu_b = class_centroid_lu(args, cat_F_lu_b, cat_Y_lu_b)

        Loss_test += ce_loss(logits_b, y_subset_b)
...
def test_fcn(args, batImageGenTest, FT, CLS):
    ...
    if len(images_test) > threshold:
        ...
        for test_image_split in test_image_splits:
            ...
            images_test_split = Variable(images_test_split, requires_grad=False)

            Feat_ts = FT.features(images_test_split)
            classifier_out = CLS(Feat_ts)
            test_image_preds.append(classifier_out.detach().numpy())
        ...
        images_test = Variable(images_test, requires_grad=False)
        Feat_ts = FT.features(images_test)
        classifier_out = CLS(Feat_ts)
        predictions = classifier_out.detach().numpy()
