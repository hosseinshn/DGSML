...
def scoring(prob_pred):
    w = torch.from_numpy(entropy(prob_pred.detach().numpy(), base=10, axis=-1).astype(np.float32))
    score = 1-w
    return score
...
def cat_dist_mats(args, mat1, mat2):
    number_class = args.num_classes
    cat_mat = torch.zeros([(number_class*(len(mat1)+len(mat2))), mat1[0].size(1)])
    ind = np.linspace(0, number_class-1, number_class).astype(np.int64)
